...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        with fh:
            ...
            if unknown:
                self.stdout.write(
                    self.style.WARNING(f"Ignoring unknown column(s): {', '.join(sorted(unknown))}")
                )

            created = skipped = errors = 0
            existing_tsc = set(Member.objects.values_list("tsc_number", flat=True))
        ...
        for i, row in enumerate(rows, start=2):  # row 1 is the header
            ...
            if not tsc or not row.get("first_name") or not row.get("last_name"):
                self.stdout.write(self.style.ERROR(f"Row {i}: missing required value, skipped."))
                errors += 1
                continue

            if tsc in existing_tsc:
                self.stdout.write(f"Row {i}: tsc_number {tsc} already exists, skipped.")
                skipped += 1
                continue
            ...
            if sub_county and sub_county not in _VALID_SUBCOUNTIES:
                self.stdout.write(
                    self.style.WARNING(f'Row {i}: unknown sub_county "{sub_county}", left blank.')
                )
                sub_county = ""
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Row {i}: failed ({exc}), skipped."))
                errors += 1

        verb = "Would create" if dry else "Created"
        self.stdout.write(
            self.style.SUCCESS(f"{verb} {created}, skipped {skipped} existing, {errors} errors.")
        )
